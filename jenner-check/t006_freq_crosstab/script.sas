/* Adapted from Working_code_cleaned.sas (amonkhouse11/SQL-Practice).
   The source derives PHQ2/TBI2 dichotomous flags in its DATA step recode,
   then runs PROC FREQ with a BY group and a crosstab TABLES statement.
   The recode + PROC FREQ steps below match the source; `nhanes` is a
   small inline mock dataset with the same columns. */
...
proc sort data=nhanes; by ridreth1; run;
