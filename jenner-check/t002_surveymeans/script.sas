/* Adapted from Working_code_cleaned.sas (amonkhouse11/SQL-Practice).
   The original builds `nhanes` from `libname thesis "D:\Thesis";` reads
   (crp/demo/phq9/ferritin/tfr/tbi/bmi merged by seqn) before running
   PROC SURVEYMEANS with the survey weight/strata/cluster design below.
   Here `nhanes` is a small inline mock dataset with the same columns;
   the PROC SURVEYMEANS step is otherwise unchanged from the source. */
...
strata sddsrvyr;
cluster sdmvpsu;
var  PHQ9 TBI LBDFERSI LBXTFR lbxcrp BMXBMI;
where tbi ^=.;
where phq9 ^=.;
where LBDFERSI ^=.;
where LBXTFR ^=.;
