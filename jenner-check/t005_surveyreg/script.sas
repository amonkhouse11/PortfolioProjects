/* Adapted from Working_code_cleaned.sas (amonkhouse11/SQL-Practice).
   The source builds hispanic/black/other dummy variables from ridreth1
   inside its DATA step recode, then models PHQ9 with PROC SURVEYREG.
   The recode + PROC SURVEYREG model below match the source; `nhanes`
   is a small inline mock dataset with the same columns. */
